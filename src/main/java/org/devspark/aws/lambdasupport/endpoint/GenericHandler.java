...
public abstract class GenericHandler {
    ...
    public void handle(InputStream in, OutputStream out, Context context)
        ...
        if (action.isEmpty()) {
            throw new BadRequestException("No action identifier was provided in payload");
        }

        ActionRegistryEntry actionRegistryEntry = actionRegistryEntries.get(action);
        ...
        if (actionRegistryEntry.getPayloadClass() != null && payload == null) {
            throw new BadRequestException("Payload expected for action -> " + action);
        }
